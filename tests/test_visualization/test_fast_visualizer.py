...
class TestFastVisualizer(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.visualizer.keypoint_id2name, self.metainfo["keypoint_id2name"])
        self.assertEqual(self.visualizer.keypoint_name2id, self.metainfo["keypoint_name2id"])
        np.testing.assert_array_equal(self.visualizer.keypoint_colors, self.metainfo["keypoint_colors"])
        self.assertEqual(self.visualizer.skeleton_links, self.metainfo["skeleton_links"])
        np.testing.assert_array_equal(self.visualizer.skeleton_link_colors, self.metainfo["skeleton_link_colors"])

    def test_draw_pose(self):
        img = np.zeros((480, 640, 3), dtype=np.uint8)
        instances = type("Instances", (object,), {})()
        instances.keypoints = np.array([[[100, 100], [200, 200], [300, 300]]], dtype=np.float32)
