...
class TestPoseLocalVisualizer(TestCase):
    ...
    def test_add_datasample(self):
        ...
        gt_instances.keypoints = np.array([[[1, 1], [20, 20], [40, 40], [80, 80]]], dtype=np.float32)
        ...
        self.visualizer.add_datasample(
            "image", image, data_sample=pred_pose_data_sample, draw_bbox=True, out_file=out_file
        )
        self._assert_image_and_shape(out_file, (h, w * 2, 3))

        self.visualizer.show_keypoint_weight = False
        self.visualizer.add_datasample(
            "image", image, data_sample=pred_pose_data_sample, draw_pred=False, draw_heatmap=True, out_file=out_file
        )
        self._assert_image_and_shape(out_file, ((h * 2), w, 3))

        self.visualizer.add_datasample(
            "image", image, data_sample=pred_pose_data_sample, draw_heatmap=True, out_file=out_file
        )
