...
class TestMSRAHeatmap(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.configs = [
            (
                "msra",
                dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0),
            ),
            (
                "msra+dark",
                dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0, unbiased=True),
            ),
        ]
        ...
        self.data = dict(keypoints=keypoints, keypoints_visible=keypoints_visible, heatmaps=heatmaps)
    ...
    def test_encode(self):
        ...
        for name, cfg in self.configs:
            ...
            self.assertEqual(encoded["heatmaps"].shape, (17, 64, 48), f'Failed case: "{name}"')
            self.assertEqual(encoded["keypoint_weights"].shape, (1, 17)), f'Failed case: "{name}"'
