...
class TestKeypointConverter(TestCase):
    ...
    def test_transform(self):
        ...
        # check shape
        self.assertEqual(results["keypoints"].shape[0], self.data_info["keypoints"].shape[0])
        self.assertEqual(results["keypoints"].shape[1], 5)
        self.assertEqual(results["keypoints"].shape[2], 2)
        self.assertEqual(results["keypoints_visible"].shape[0], self.data_info["keypoints_visible"].shape[0])
        self.assertEqual(results["keypoints_visible"].shape[1], 5)

        # check value
        for source_index, target_index in mapping:
            self.assertTrue(
                (results["keypoints"][:, target_index] == self.data_info["keypoints"][:, source_index]).all()
            )
            ...
            if isinstance(source_index, tuple):
                ...
                self.assertTrue(
                    (
                        ...
                        == 0.5
                        * (self.data_info["keypoints"][:, source_index] + self.data_info["keypoints"][:, source_index2])
                    ).all()
                )
                self.assertEqual(results["keypoints_visible"].ndim, 3)
                self.assertEqual(results["keypoints_visible"].shape[2], 2)
        ...
        # check shape
        self.assertEqual(results["keypoints_3d"].shape[0], self.data_info["keypoints_3d"].shape[0])
        ...
        for source_index, target_index in mapping:
            self.assertTrue(
                (results["keypoints_3d"][:, target_index] == self.data_info["keypoints_3d"][:, source_index]).all()
            )
            self.assertEqual(results["keypoints_visible"].ndim, 3)
            self.assertEqual(results["keypoints_visible"].shape[2], 2)
            self.assertTrue(
