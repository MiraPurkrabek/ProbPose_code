...
def get_h36m_sample():
    ...
    ann_info["flip_indices"] = [0, 4, 5, 6, 1, 2, 3, 7, 8, 9, 10, 14, 15, 16, 11, 12, 13]
...
class TestRandomFlipAroundRoot(TestCase):
    ...
    def test_init(self):
        _ = RandomFlipAroundRoot(self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=0.5, flip_camera=False)
    ...
    def test_transform(self):
        ...
        transform = RandomFlipAroundRoot(self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=1)
        ...
        for left, right in enumerate(flip_indices):
            self.assertTrue(np.allclose(-kpts1[0][left][:1], kpts2[0][right][:1], atol=4.0))
            self.assertTrue(np.allclose(kpts1[0][left][1:], kpts2[0][right][1:], atol=4.0))
            self.assertTrue(np.allclose(tar1[..., left, 1:], tar2[..., right, 1:], atol=4.0))

            self.assertTrue(np.allclose(kpts_vis1[..., left], kpts_vis2[..., right], atol=4.0))
            self.assertTrue(np.allclose(tar_vis1[..., left], tar_vis2[..., right], atol=4.0))

        # test camera flipping
        transform = RandomFlipAroundRoot(self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=1, flip_camera=True)
        results = deepcopy(self.data_info)
        results = transform(results)

        camera2 = results["camera_param"]
        self.assertTrue(np.allclose(-self.data_info["camera_param"]["c"][0], camera2["c"][0], atol=4.0))
        self.assertTrue(np.allclose(-self.data_info["camera_param"]["p"][0], camera2["p"][0], atol=4.0))
        ...
        transform = RandomFlipAroundRoot(self.keypoints_flip_cfg, self.target_flip_cfg, flip_prob=1, flip_label=True)
