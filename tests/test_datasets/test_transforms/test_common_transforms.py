...
class TestGetBBoxCenterScale(TestCase):
    ...
    def setUp(self):
        ...
        self.data_info = get_coco_sample(
            img_shape=(480, 640), num_instances=1, with_bbox_cs=True, with_img_mask=True, random_keypoints_visible=True
        )
...
class TestRandomFlip(TestCase):
    ...
    def test_transform(self):
        ...
        self.assertTrue(np.allclose(results["img_mask"], self.data_info["img_mask"][:, ::-1]))
        ...
        self.assertTrue(np.allclose(results["img_mask"], self.data_info["img_mask"][::-1, ::-1]))
        self.assertTrue(np.allclose(results["bbox_center"], bbox_center_flipped))
        self.assertTrue(np.allclose(kpts1[..., 0], 640 - kpts2[..., 0] - 1))
        self.assertTrue(np.allclose(kpts1[..., 1], 480 - kpts2[..., 1] - 1))
        self.assertTrue(np.allclose(kpts1_vis, kpts2_vis))
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "direction must be either str or list of str"):
...
class TestRandomHalfBody(TestCase):
    ...
    def setUp(self):
        # prepare dummy top-down data sample with COCO metainfo
        self.data_info = get_coco_sample(img_shape=(480, 640), num_instances=1, with_bbox_cs=True, with_img_mask=True)

    def test_transform(self):
        padding = 1.5

        # keep upper body
        transform = RandomHalfBody(prob=1.0, min_total_keypoints=8, min_upper_keypoints=2, min_lower_keypoints=2)
        results = deepcopy(self.data_info)
        ...
        self.assertTrue(np.allclose(results["bbox_scale"], (kpts.max(axis=1) - kpts.min(axis=1)) * padding))

        # keep lower body
        transform = RandomHalfBody(prob=1.0, min_total_keypoints=6, min_upper_keypoints=4, min_lower_keypoints=4)
        results = deepcopy(self.data_info)
        ...
        self.assertTrue(np.allclose(results["bbox_center"], self.data_info["bbox_center"]))
        self.assertTrue(np.allclose(results["bbox_scale"], self.data_info["bbox_scale"]))
        ...
        transform = RandomHalfBody(prob=1.0, min_total_keypoints=4, min_upper_keypoints=3, min_lower_keypoints=3)
        results = deepcopy(self.data_info)
...
class TestRandomBBoxTransform(TestCase):
    ...
    def test_transform(self):
        ...
        # test random shift
        transform = RandomBBoxTransform(shift_factor=shfit_factor, shift_prob=1.0, scale_prob=0.0, rotate_prob=0.0)
        results = transform(deepcopy(self.data_info))
        ...
        self.assertTrue(((results["bbox_center"] > center_range[0]) & (results["bbox_center"] < center_range[1])).all())
        self.assertTrue(np.allclose(results["bbox_scale"], scale))
        self.assertTrue(np.allclose(results["bbox_rotation"], np.zeros((1, 17))))

        # test random resizing
        transform = RandomBBoxTransform(scale_factor=scale_factor, shift_prob=0.0, scale_prob=1.0, rotate_prob=0.0)
        ...
        self.assertTrue(((results["bbox_scale"] > scale_range[0]) & (results["bbox_scale"] < scale_range[1])).all())
        self.assertTrue(np.allclose(results["bbox_rotation"], np.zeros((1, 17))))

        # test random rotation
        transform = RandomBBoxTransform(rotate_factor=rotate_factor, shift_prob=0.0, scale_prob=0.0, rotate_prob=1.0)
        ...
        self.assertTrue(
            ((results["bbox_rotation"] > bbox_rotation_min) & (results["bbox_rotation"] < bbox_rotation_max)).all()
        )
        ...
        self.assertFalse(np.allclose(results["bbox_rotation"], 0))
        self.assertTrue(
            ((results["bbox_rotation"] > rotation_range[0]) & (results["bbox_rotation"] < rotation_range[1])).all()
        )

    def test_repr(self):
        transform = RandomBBoxTransform(
            shift_factor=0.16, shift_prob=0.3, scale_factor=0.5, scale_prob=1.0, rotate_factor=40.0, rotate_prob=0.6
        )
...
class TestAlbumentation(TestCase):
    ...
    def test_transform(self):
        ...
        transform = Albumentation(transforms=[dict(type="RandomBrightnessContrast", p=0.2), dict(type="ToFloat")])
...
class TestGenerateTarget(TestCase):
    ...
    def test_generate_single_target(self):
        encoder = dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0)

        # generate heatmap
        pipeline = Compose([TopdownAffine(input_size=(192, 256)), GenerateTarget(encoder=encoder)])
        ...
        self.assertTrue(np.allclose(results["keypoint_weights"], self.data_info["dataset_keypoint_weights"][None]))

    def test_generate_multilevel_target(self):
        encoder_0 = dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0)
        ...
        pipeline = Compose(
            [
                TopdownAffine(input_size=(192, 256)),
                GenerateTarget(encoder=[encoder_0, encoder_1], multilevel=True, use_dataset_keypoint_weights=True),
            ]
        )
        results = pipeline(deepcopy(self.data_info))
    ...
    def test_generate_combined_target(self):
        ...
        pipeline = Compose(
            [
                TopdownAffine(input_size=(192, 256)),
                GenerateTarget(encoder=[encoder_0, encoder_1], multilevel=False, use_dataset_keypoint_weights=True),
            ]
        )
    ...
    def test_errors(self):
        ...
        encoder_1 = dict(type="RegressionLabel", input_size=(192, 256))
        pipeline = Compose(
            [TopdownAffine(input_size=(192, 256)), GenerateTarget(encoder=[encoder_0, encoder_1], multilevel=True)]
        )
        ...
        pipeline = Compose(
            [TopdownAffine(input_size=(192, 256)), GenerateTarget(encoder=[encoder, encoder], multilevel=False)]
        )
...
class TestFilterAnnotations(TestCase):
    ...
    def test_transform(self):
        ...
        self.assertTrue((results["bbox"] == np.array([[20, 20, 40, 40], [40, 40, 80, 80]])).all())
        self.assertTrue((results["bbox_score"] == np.array([0.8, 0.7])).all())
        self.assertTrue((results["category_id"] == np.array([2, 3])).all())
        self.assertTrue((results["keypoints"] == np.array([[25, 25, 1], [45, 45, 1]])).all())
        self.assertTrue((results["keypoints_visible"] == np.array([[1, 1, 1], [1, 1, 1]])).all())
...
class TestYOLOXHSVRandomAug(TestCase):
    ...
    def setUp(self):
        ...
        tearDown() -> cleanUp()
        """
        img = mmcv.imread(osp.join(osp.dirname(__file__), "../../data/coco/000000000785.jpg"), "color")
        self.results = {
            "img": img,
            "img_shape": (640, 425),
            "category_id": np.array([1, 2, 3], dtype=np.int64),
            "bbox": np.array([[10, 10, 20, 20], [20, 20, 40, 40], [40, 40, 80, 80]], dtype=np.float32),
        }
    ...
    def test_repr(self):
        transform = YOLOXHSVRandomAug()
        self.assertEqual(repr(transform), ("YOLOXHSVRandomAug(hue_delta=5, " "saturation_delta=30, " "value_delta=30)"))
