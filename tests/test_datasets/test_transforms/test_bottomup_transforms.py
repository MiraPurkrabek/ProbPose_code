...
class TestBottomupRandomAffine(TestCase):
    ...
    def setUp(self):
        ...
        self.data_info = get_coco_sample(img_shape=(240, 320), num_instances=4, with_bbox_cs=True)
...
class TestBottomupGetHeatmapMask(TestCase):
    ...
    def test_transform(self):
        ...
        results["heatmaps"] = [np.random.rand(17, h, w).astype(np.float32) for w, h in heatmap_sizes]
        results = pipeline(results)
...
class TestBottomupResize(TestCase):
    ...
    def setUp(self):
        # prepare dummy bottom-up data sample with COCO metainfo
        self.data_info = get_coco_sample(img_shape=(240, 480), img_fill=255, num_instances=4, with_bbox_cs=True)
    ...
    def test_transform(self):
        ...
        transform = BottomupResize(input_size=(256, 256), resize_mode="expand", size_factor=100)
        results = transform(deepcopy(self.data_info))
        # input size is ceiled from (512, 256) to (600, 300)
        self.assertEqual(results["img"].shape, (300, 600, 3))

        # multi-scale
        transform = BottomupResize(input_size=(256, 256), aug_scales=[1.5], resize_mode="fit")
        results = transform(deepcopy(self.data_info))
...
class TestBottomupRandomCrop(TestCase):
    ...
    def test_transform(self):
        ...
        target_shape = (12, 16)
        for crop_type, crop_size in zip(["relative", "absolute"], [(0.5, 0.5), (16, 12)]):
        ...
        transform = BottomupRandomCrop(
            crop_size=(7, 5), allow_negative_crop=False, recompute_bbox=False, bbox_clip_border=True
        )
        results = transform(deepcopy(src_results))
        h, w = results["img"].shape[:2]
...
class TestBottomupRandomChoiceResize(TestCase):
    ...
    def test_transform(self):
        ...
        # test keep_ratio
        transform = BottomupRandomChoiceResize(scales=[(900, 600)], resize_type="Resize", keep_ratio=True)
        results = deepcopy(self.data_info)
        ...
        transform = BottomupRandomChoiceResize(scales=[(200, 150)], resize_type="Resize", clip_object_border=True)
        results = deepcopy(self.data_info)
        ...
        transform = BottomupRandomChoiceResize(scales=[(200, 150)], resize_type="Resize", clip_object_border=False)
