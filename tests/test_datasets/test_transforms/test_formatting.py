...
class TestPackPoseInputs(TestCase):
    ...
    def setUp(self):
        ...
        self.results_topdown = {
            ...
            "keypoint_x_labels": np.random.randint(0, 100, (1, 17, 2)).astype(np.float32),
            "keypoint_y_labels": np.random.randint(0, 100, (1, 17, 2)).astype(np.float32),
            "transformed_keypoints": np.random.randint(0, 100, (1, 17, 2)).astype(np.float32),
        }
        self.meta_keys = ("img_id", "img_path", "ori_shape", "img_shape", "scale_factor", "flip", "flip_direction")
    ...
    def test_transform(self):
        ...
        len_seq = 5
        results["img"] = [np.random.randint(0, 255, (425, 640, 3), dtype=np.uint8) for _ in range(len_seq)]
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(transform),
            f"PackPoseInputs(meta_keys={self.meta_keys}, " f"pack_transformed={transform.pack_transformed})",
        )
