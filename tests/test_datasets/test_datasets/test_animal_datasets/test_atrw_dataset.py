...
class TestATRWDataset(TestCase):
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported when `test_mode==True`'):
            _ = self.build_atrw_dataset(data_mode="topdown", test_mode=False, bbox_file="temp_bbox_file.json")

        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported in topdown mode'):
            _ = self.build_atrw_dataset(data_mode="bottomup", test_mode=True, bbox_file="temp_bbox_file.json")

        with self.assertRaisesRegex(ValueError, '"bbox_score_thr" is only supported in topdown mode'):
            _ = self.build_atrw_dataset(data_mode="bottomup", test_mode=True, filter_cfg=dict(bbox_score_thr=0.3))
