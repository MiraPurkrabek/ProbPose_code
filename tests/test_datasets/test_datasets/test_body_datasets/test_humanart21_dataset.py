...
class TestHumanart21Dataset(TestCase):
    ...
    def test_topdown(self):
        ...
        dataset = self.build_humanart_dataset(
            data_mode="topdown", test_mode=True, bbox_file="tests/data/humanart/test_humanart_det_AP_H_56.json"
        )
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported when `test_mode==True`'):
            _ = self.build_humanart_dataset(
                data_mode="topdown", test_mode=False, bbox_file="tests/data/humanart/test_humanart_det_AP_H_56.json"
            )

        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported in topdown mode'):
            _ = self.build_humanart_dataset(
                data_mode="bottomup", test_mode=True, bbox_file="tests/data/humanart/test_humanart_det_AP_H_56.json"
            )

        with self.assertRaisesRegex(ValueError, '"bbox_score_thr" is only supported in topdown mode'):
            _ = self.build_humanart_dataset(data_mode="bottomup", test_mode=True, filter_cfg=dict(bbox_score_thr=0.3))
