...
class TestCrowdPoseDataset(TestCase):
    ...
    def test_topdown(self):
        ...
        dataset = self.build_crowdpose_dataset(
            data_mode="topdown", test_mode=True, bbox_file="tests/data/crowdpose/test_crowdpose_det_AP_40.json"
        )
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported when `test_mode==True`'):
            _ = self.build_crowdpose_dataset(
                data_mode="topdown", test_mode=False, bbox_file="tests/data/crowdpose/test_crowdpose_det_AP_40.json"
            )

        with self.assertRaisesRegex(ValueError, '"bbox_file" is only supported in topdown mode'):
            _ = self.build_crowdpose_dataset(
                data_mode="bottomup", test_mode=True, bbox_file="tests/data/crowdpose/test_crowdpose_det_AP_40.json"
            )

        with self.assertRaisesRegex(ValueError, '"bbox_score_thr" is only supported in topdown mode'):
            _ = self.build_crowdpose_dataset(data_mode="bottomup", test_mode=True, filter_cfg=dict(bbox_score_thr=0.97))
