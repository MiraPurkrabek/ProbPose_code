...
class TestH36MDataset(TestCase):
    ...
    def test_topdown(self):
        ...
        dataset = self.build_h36m_dataset(data_mode="topdown", camera_param_file="cameras.pkl")
        self.assertEqual(len(dataset), 4)
        self.check_data_info_keys(dataset[0])
    ...
    def test_bottomup(self):
        ...
        # test bottomup training
        dataset = self.build_h36m_dataset(
            data_mode="bottomup", seq_len=27, seq_step=1, causal=False, pad_video_seq=True
        )
        self.assertEqual(len(dataset), 4)
        self.check_data_info_keys(dataset[0], data_mode="bottomup")
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(
            ValueError, 'Unsupported `keypoint_2d_src` "invalid". ' f"Supported options are {SUPPORTED_keypoint_2d_src}"
        ):
            _ = self.build_h36m_dataset(data_mode="topdown", test_mode=False, keypoint_2d_src="invalid")

        with self.assertRaisesRegex(AssertionError, "Annotation file `(.+?)` does not exist"):
            _ = self.build_h36m_dataset(data_mode="topdown", test_mode=False, ann_file="invalid")
