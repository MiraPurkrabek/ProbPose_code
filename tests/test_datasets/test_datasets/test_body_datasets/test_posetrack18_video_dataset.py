...
class TestPoseTrack18VideoDataset(TestCase):
    ...
    def test_topdown(self):
        ...
        dataset = self.build_posetrack18_video_dataset(data_mode="topdown", frame_sampler_mode="random")
        self.assertEqual(len(dataset), 14)
        self.check_data_info_keys(dataset[0])

        # test topdown training, frame_sampler_mode = 'fixed'
        dataset = self.build_posetrack18_video_dataset(
            data_mode="topdown", frame_sampler_mode="fixed", frame_weights=[0.0, 1.0], frame_indices=[-1, 0]
        )
        self.assertEqual(len(dataset), 14)
        self.check_data_info_keys(dataset[0])

        # test topdown testing, frame_sampler_mode = 'random'
        dataset = self.build_posetrack18_video_dataset(data_mode="topdown", test_mode=True, frame_sampler_mode="random")
        self.assertEqual(len(dataset), 14)
        self.check_data_info_keys(dataset[0])
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(AssertionError, "frame_weight can not be a negative value"):
        ...
        with self.assertRaisesRegex(AssertionError, "frame_range can not be a negative value"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
                frame_range=-2,
            )
        ...
        with self.assertRaisesRegex(TypeError, "The type of `frame_range` must be int or Sequence"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
        ...
        with self.assertRaisesRegex(AssertionError, "please specify `num_sampled_frame`"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
                num_sampled_frame=None,
            )
        with self.assertRaisesRegex(AssertionError, "does not match the number of sampled adjacent frames"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="random",
        ...
        with self.assertRaisesRegex(AssertionError, "please specify the `frame_indices`"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="fixed",
                frame_indices=None,
            )
        with self.assertRaisesRegex(AssertionError, "does not match the length of frame_indices"):
            _ = self.build_posetrack18_video_dataset(
                frame_sampler_mode="fixed",
