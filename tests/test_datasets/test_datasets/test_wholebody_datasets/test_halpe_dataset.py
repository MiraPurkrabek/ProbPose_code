...
class TestHalpeDataset(TestCase):
    ...
    def test_topdown(self):
        ...
        dataset = self.build_halpe_dataset(test_mode=True, bbox_file="tests/data/coco/test_coco_det_AP_H_56.json")
        self.assertEqual(dataset.data_mode, "topdown")
        ...
        dataset = self.build_halpe_dataset(
            test_mode=True, bbox_file="tests/data/coco/test_coco_det_AP_H_56.json", filter_cfg=dict(bbox_score_thr=0.3)
        )
        self.assertEqual(dataset.data_mode, "topdown")
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(ValueError, '"bbox_score_thr" is only supported in topdown mode'):
            _ = self.build_halpe_dataset(data_mode="bottomup", test_mode=True, filter_cfg=dict(bbox_score_thr=0.3))
