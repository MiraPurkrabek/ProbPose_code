...
class TestH36MWholeBodyDataset(TestCase):
    ...
    def build_h3wb_dataset(self, **kwargs):
        ...
        cfg = dict(ann_file="h3wb_train_sub.npz", data_mode="topdown", data_root="tests/data/h3wb", pipeline=[])
    ...
    def check_data_info_keys(self, data_info: dict):
        expected_keys = dict(
            img_paths=list, keypoints=np.ndarray, keypoints_3d=np.ndarray, scale=np.ndarray, center=np.ndarray, id=int
        )
    ...
    def test_topdown(self):
        ...
        dataset = self.build_h3wb_dataset(data_mode="topdown", seq_len=1, seq_step=1, causal=False, pad_video_seq=True)
        dataset.full_init()
        self.assertEqual(len(dataset), 3)
        self.check_data_info_keys(dataset[0])
