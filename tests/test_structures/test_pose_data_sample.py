...
class TestPoseDataSample(TestCase):
    ...
    def get_pose_data_sample(self, multilevel: bool = False):
        ...
        if multilevel:
            ...
            gt_fields = MultilevelPixelData(metainfo=metainfo, heatmaps=heatmaps, masks=masks)
        else:
            gt_fields = PixelData()
    ...
    def test_setter(self):
        ...
        pred_instances_data = dict(keypoints=torch.rand(1, 17, 2), scores=torch.rand(1, 17, 1))
        data_sample.pred_instances = InstanceData(**pred_instances_data)

        self.assertTrue(self._equal(data_sample.pred_instances.keypoints, pred_instances_data["keypoints"]))
        self.assertTrue(self._equal(data_sample.pred_instances.scores, pred_instances_data["scores"]))
        ...
        self.assertTrue(self._equal(data_sample.pred_fields.heatmaps, pred_fields_data["heatmaps"]))

        # test to_tensor
        data_sample = data_sample.to_tensor()
        self.assertTrue(self._equal(data_sample.pred_fields.heatmaps, torch.from_numpy(pred_fields_data["heatmaps"])))
