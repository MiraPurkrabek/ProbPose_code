...
class TestMMPoseInferencer(TestCase):
    ...
    def test_pose2d_call(self):
        ...
        inferencer = MMPoseInferencer("human", det_model=det_model, det_weights=det_weights)
        ...
        self.assertSequenceEqual(results1["predictions"][0][0]["keypoints"], results3["predictions"][3][0]["keypoints"])

        # `inputs` is path to a video
        inputs = "tests/data/posetrack18/videos/000001_mpiinew_test/" "000001_mpiinew_test.mp4"
        with TemporaryDirectory() as tmp_dir:
            results = defaultdict(list)
            for res in inferencer(inputs, out_dir=tmp_dir):
                for key in res:
                    results[key].extend(res[key])
            self.assertIn("000001_mpiinew_test.mp4", os.listdir(f"{tmp_dir}/visualizations"))
            self.assertIn("000001_mpiinew_test.json", os.listdir(f"{tmp_dir}/predictions"))
    ...
    def test_pose3d_call(self):
        ...
        if platform.system().lower() == "windows":
            ...
            det_model, det_weights = None, None
        inferencer = MMPoseInferencer(pose3d="human3d", det_model=det_model, det_weights=det_weights)

        # `inputs` is path to a video
        inputs = (
            "https://user-images.githubusercontent.com/87690686/" "164970135-b14e424c-765a-4180-9bc8-fa8d6abc5510.mp4"
        )
        with TemporaryDirectory() as tmp_dir:
            results = defaultdict(list)
            for res in inferencer(inputs, out_dir=tmp_dir):
                for key in res:
                    results[key].extend(res[key])
            self.assertIn("164970135-b14e424c-765a-4180-9bc8-fa8d6abc5510.mp4", os.listdir(f"{tmp_dir}/visualizations"))
            self.assertIn("164970135-b14e424c-765a-4180-9bc8-fa8d6abc5510.json", os.listdir(f"{tmp_dir}/predictions"))
        self.assertTrue(inferencer._video_input)
