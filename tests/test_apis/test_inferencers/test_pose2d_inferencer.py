...
class TestPose2DInferencer(TestCase):
    ...
    def test_init(self):
        ...
        inferencer = Pose2DInferencer(
            model="configs/body_2d_keypoint/simcc/coco/" "simcc_res50_8xb64-210e_coco-256x192.py",
        ...
        self.assertIsInstance(inferencer.model, torch.nn.Module)
        self.assertIsInstance(inferencer.detector, BaseInferencer)
        self.assertSequenceEqual(inferencer.det_cat_ids, (15, 16, 17, 18, 19, 20, 21, 22, 23))

        # 4. init with bottom-up model
        inferencer = Pose2DInferencer(
            model="configs/body_2d_keypoint/dekr/coco/" "dekr_hrnet-w32_8xb10-140e_coco-512x512.py",
    ...
    def test_call(self):
        ...
        # top-down model
        det_model, det_weights = self._get_det_model_weights()
        inferencer = Pose2DInferencer("human", det_model=det_model, det_weights=det_weights)
