...
class TestPose3DInferencer(TestCase):
    ...
    def test_init(self):
        ...
        inferencer = Pose3DInferencer(
            ...
            pose2d_model="configs/body_2d_keypoint/simcc/coco/" "simcc_res50_8xb64-210e_coco-256x192.py",
        ...
        # 3. init with alias
        inferencer = Pose3DInferencer(
            model="human3d", det_model=det_model, det_weights=det_weights, det_cat_ids=0 if det_model else None
        )
        self.assertIsInstance(inferencer.model, torch.nn.Module)
        self.assertIsInstance(inferencer.pose2d_model, Pose2DInferencer)
