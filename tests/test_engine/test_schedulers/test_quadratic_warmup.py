...
from mmpose.engine.schedulers import QuadraticWarmupLR, QuadraticWarmupMomentum, QuadraticWarmupParamScheduler
...
class TestQuadraticWarmupScheduler(TestCase):
    ...
    def setUp(self):
        ...
        """
        self.model = ToyModel()
        self.optimizer = optim.SGD(self.model.parameters(), lr=0.05, momentum=0.01, weight_decay=5e-4)
    ...
    def test_quadratic_warmup_scheduler(self):
        ...
        scheduler = QuadraticWarmupParamScheduler(self.optimizer, param_name="lr", end=iters)
    ...
    def test_quadratic_warmup_scheduler_convert_iterbased(self):
        ...
        warmup_factor = [pow((i + 1) / float(iters), 2) for i in range(iters)]
        single_targets = [x * 0.05 for x in warmup_factor] + [0.05] * (epochs * epoch_length - iters)
        targets = [single_targets, [x * epochs for x in single_targets]]
