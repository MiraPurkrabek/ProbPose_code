...
from mmpose.models.losses.heatmap_loss import AdaptiveWingLoss, FocalHeatmapLoss, KeypointMSELoss, MLECCLoss
...
class TestAdaptiveWingLoss(TestCase):
    ...
    def test_loss(self):
        ...
        fake_label = torch.zeros((1, 3, 2, 2))
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label), torch.tensor(8.4959), atol=1e-4))
        ...
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label, fake_weight), torch.tensor(0.0)))
...
class TestFocalHeatmapLoss(TestCase):
    ...
    def test_loss(self):
        ...
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label), torch.tensor(0.1569), atol=1e-4))

        # test loss w/ target_weight
        loss = FocalHeatmapLoss(use_target_weight=True)

        fake_weight = torch.arange(3 * 5 * 5).reshape(1, 3, 5, 5).float()
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label, fake_weight), torch.tensor(5.8062), atol=1e-4))
...
class TestKeypointMSELoss(TestCase):
    ...
    def test_loss(self):
        ...
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label), torch.tensor(0.0), atol=1e-4))
        ...
        self.assertTrue(torch.allclose(loss(fake_pred, fake_label, fake_weight), torch.tensor(0.0), atol=1e-4))
        ...
        self.assertTrue(
            torch.allclose(loss(fake_pred, fake_label, fake_weight, fake_mask), torch.tensor(0.0), atol=1e-4)
        )
