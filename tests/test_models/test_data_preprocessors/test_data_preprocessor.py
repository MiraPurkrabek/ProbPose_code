...
class TestPoseDataPreprocessor(TestCase):
    ...
    def test_forward(self):
        ...
        data = {"inputs": [torch.randint(0, 256, (3, 11, 10))], "data_samples": [PoseDataSample()]}
        ...
        processor = PoseDataPreprocessor(mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0], bgr_to_rgb=True)
        out_data = processor(data)
        batch_inputs, batch_data_samples = out_data["inputs"], out_data["data_samples"]
        self.assertEqual(batch_inputs.shape, (1, 3, 11, 10))
        self.assertEqual(len(batch_data_samples), 1)
        ...
        # test padding
        data = {
            "inputs": [torch.randint(0, 256, (3, 10, 11)), torch.randint(0, 256, (3, 9, 14))],
        ...
        data = {
            "inputs": [torch.randint(0, 256, (3, 10, 11)), torch.randint(0, 256, (3, 9, 24))],
            "data_samples": [PoseDataSample()] * 2,
        }
        processor = PoseDataPreprocessor(mean=[0.0, 0.0, 0.0], std=[1.0, 1.0, 1.0], pad_size_divisor=5)
        out_data = processor(data)
        batch_inputs, batch_data_samples = out_data["inputs"], out_data["data_samples"]
        self.assertEqual(batch_inputs.shape, (2, 3, 10, 25))
        self.assertEqual(len(batch_data_samples), 2)
        for data_samples, expected_shape in zip(batch_data_samples, [(10, 15), (10, 25)]):
            self.assertEqual(data_samples.pad_shape, expected_shape)

    def test_batch_sync_random_resize(self):
        processor = PoseDataPreprocessor(
            batch_augments=[
                dict(type="BatchSyncRandomResize", random_size_range=(320, 320), size_divisor=32, interval=1)
            ]
        )
        ...
        packed_inputs = {
            "inputs": [torch.randint(0, 256, (3, 128, 128)), torch.randint(0, 256, (3, 128, 128))],
            "data_samples": [PoseDataSample()] * 2,
        }
