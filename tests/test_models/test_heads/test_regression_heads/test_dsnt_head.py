...
class TestDSNTHead(TestCase):
    ...
    def test_init(self):
        ...
        head = DSNTHead(
            ...
            in_featuremap_size=(6, 8),
            num_joints=17,
            decoder=dict(type="IntegralRegressionLabel", input_size=(192, 256), heatmap_size=(48, 64), sigma=2),
        )

        self.assertIsNotNone(head.decoder)

    def test_predict(self):
        decoder_cfg = dict(type="IntegralRegressionLabel", input_size=(192, 256), heatmap_size=(48, 64), sigma=2)
        ...
        batch_data_samples = get_packed_inputs(batch_size=2, with_reg_label=False)["data_samples"]
    ...
    def test_loss(self):
        for dist_loss in ["l1", "l2"]:
            ...
            feats = self._get_feats(batch_size=2, feat_shapes=[(32, 8, 6)])
            batch_data_samples = get_packed_inputs(batch_size=2, with_reg_label=True)["data_samples"]
