...
class TestVisPredictHead(TestCase):
    ...
    def test_init(self):
        codec = dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0)
    ...
    def test_predict(self):
        ...
        self.assertEqual(
            preds[0].keypoint_scores.shape, batch_data_samples[0].gt_instance_labels.keypoint_weights.shape
        )

        # output heatmap
        head = VisPredictHead(pose_cfg=dict(type="HeatmapHead", in_channels=32, out_channels=17, decoder=codec))
    ...
    def test_tta(self):
        ...
        head = VisPredictHead(pose_cfg=dict(type="HeatmapHead", in_channels=32, out_channels=17, decoder=decoder_cfg))
