...
class TestCPMHead(TestCase):
    ...
    def _get_feats(self, batch_size: int = 2, feat_shapes: List[Tuple[int, int, int]] = [(17, 32, 24)]):
    ...
    def test_init(self):
        # w/o deconv
        head = CPMHead(num_stages=1, in_channels=256, out_channels=17, deconv_out_channels=None)
        ...
        # w/ deconv
        head = CPMHead(
            num_stages=1, in_channels=32, out_channels=17, deconv_out_channels=(32, 32), deconv_kernel_sizes=(4, 4)
        )
        self.assertTrue(isinstance(head.multi_deconv_layers, nn.ModuleList))
        ...
        head = CPMHead(
            ...
            in_channels=32,
            out_channels=17,
            decoder=dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0),
        )
    ...
    def test_predict(self):
        decoder_cfg = dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(24, 32), sigma=2.0)

        # num_stages = 6, final_layer = None
        head = CPMHead(num_stages=6, in_channels=17, out_channels=17, final_layer=None, decoder=decoder_cfg)
        ...
        head = CPMHead(
            num_stages=1, in_channels=32, out_channels=17, final_layer=dict(kernel_size=1), decoder=decoder_cfg
        )
    ...
    def test_loss(self):
        # num_stages = 1
        head = CPMHead(num_stages=1, in_channels=32, out_channels=17, final_layer=dict(kernel_size=1))
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "Got mismatched lengths"):
            _ = CPMHead(
                num_stages=1, in_channels=17, out_channels=17, deconv_out_channels=(256,), deconv_kernel_sizes=(4, 4)
            )

        with self.assertRaisesRegex(ValueError, "Unsupported kernel size"):
            _ = CPMHead(
                num_stages=1, in_channels=17, out_channels=17, deconv_out_channels=(256,), deconv_kernel_sizes=(1,)
            )
