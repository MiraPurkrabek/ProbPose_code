...
class TestCIDHead(TestCase):
    ...
    def test_loss(self):
        ...
        codec_cfg = dict(type="DecoupledHeatmap", input_size=(512, 512), heatmap_size=(128, 128))
        ...
        encoded = head.decoder.encode(data["keypoints"], data["keypoints_visible"], data["bbox"])
        feats = self._get_feats()
        data_samples = self._get_data_samples()
        for data_sample in data_samples:
            ...
            data_sample.gt_instance_labels.set_data({"instance_coords": to_tensor(encoded["instance_coords"])})
            data_sample.gt_instance_labels.set_data({"keypoint_weights": to_tensor(encoded["keypoint_weights"])})
