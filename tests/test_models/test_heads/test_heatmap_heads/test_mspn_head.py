...
class TestMSPNHead(TestCase):
    ...
    def _get_feats(
        ...
        for i in range(num_stages):
            ...
            for j in range(num_units):
                feats_units.append(torch.rand((batch_size,) + feat_shapes[j], dtype=torch.float32))
            feats_stages.append(feats_units)

        return feats_stages

    def _get_data_samples(self, batch_size: int = 2, heatmap_size=(48, 64), num_levels=1):
    ...
    def test_init(self):
        ...
        head = MSPNHead(
            ...
            decoder=dict(type="MegviiHeatmap", input_size=(192, 256), heatmap_size=(48, 64), kernel_size=11),
        )
    ...
    def test_loss(self):
        ...
        with self.assertRaisesRegex(AssertionError, "length of feature maps did not match the `num_stages`"):
            ...
            batch_data_samples = self._get_data_samples(batch_size=2, heatmap_size=(48, 64), num_levels=8)
            _ = head.loss(feats, batch_data_samples)

        with self.assertRaisesRegex(AssertionError, "length of feature maps did not match the `num_units`"):
            feats = self._get_feats(
                num_stages=1, num_units=2, batch_size=2, feat_shapes=[(unit_channels, 8, 6), (unit_channels, 16, 12)]
            )
            batch_data_samples = self._get_data_samples(batch_size=2, heatmap_size=(48, 64), num_levels=2)
            _ = head.loss(feats, batch_data_samples)

        with self.assertRaisesRegex(AssertionError, "number of feature map channels did not match"):
            feats = self._get_feats(
                num_stages=1,
                num_units=4,
                batch_size=2,
            ...
            batch_data_samples = self._get_data_samples(batch_size=2, heatmap_size=(48, 64), num_levels=4)
            _ = head.loss(feats, batch_data_samples)
        ...
        head = MSPNHead(
            ...
            loss=(
                ...
                + [dict(type="KeypointOHKMMSELoss", use_target_weight=True, loss_weight=0.1)]
            )
            * 4,
        ...
        batch_data_samples = self._get_data_samples(batch_size=2, heatmap_size=(48, 64), num_levels=16)
        losses = head.loss(feats, batch_data_samples)
    ...
    def test_predict(self):
        decoder_cfg = dict(type="MegviiHeatmap", input_size=(192, 256), heatmap_size=(48, 64), kernel_size=11)
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "The length of level_indices"):
            _ = MSPNHead(
                num_stages=2, num_units=4, out_shape=(64, 48), unit_channels=256, out_channels=17, level_indices=[0]
            )
