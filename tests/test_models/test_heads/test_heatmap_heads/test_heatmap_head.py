...
class TestHeatmapHead(TestCase):
    ...
    def _get_feats(self, batch_size: int = 2, feat_shapes: List[Tuple[int, int, int]] = [(32, 8, 6)]):
    ...
    def test_init(self):
        ...
        head = HeatmapHead(in_channels=32, out_channels=17, deconv_out_channels=(32, 32), deconv_kernel_sizes=(4, 4))
    ...
    def test_predict(self):
        decoder_cfg = dict(type="MSRAHeatmap", input_size=(192, 256), heatmap_size=(48, 64), sigma=2.0)
    ...
    def test_tta(self):
        ...
        decoder_cfg = dict(type="UDPHeatmap", input_size=(192, 256), heatmap_size=(48, 64), heatmap_type="combined")
    ...
    def test_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "Got mismatched lengths"):
            _ = HeatmapHead(in_channels=32, out_channels=17, deconv_out_channels=(256,), deconv_kernel_sizes=(4, 4))
        ...
        with self.assertRaisesRegex(ValueError, "Unsupported kernel size"):
            _ = HeatmapHead(in_channels=16, out_channels=17, deconv_out_channels=(256,), deconv_kernel_sizes=(1,))

    def test_state_dict_compatible(self):
        # Typical setting for HRNet
        head = HeatmapHead(in_channels=32, out_channels=17, deconv_out_channels=None)

        state_dict = {"final_layer.weight": torch.zeros((17, 32, 1, 1)), "final_layer.bias": torch.zeros((17))}
