...
from mmpose.evaluation.metrics import AUC, EPE, NME, JhmdbPCKAccuracy, MpiiPCKAccuracy, PCKAccuracy
...
class TestPCKAccuracy(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(KeyError, "Should be one of 'bbox', 'head', 'torso'"):
...
class TestNME(TestCase):
    ...
    def _generate_data(self, batch_size: int = 1, num_keypoints: int = 5, norm_item: str = "box_size") -> tuple:
    ...
    def test_nme_evaluate(self):
        ...
        data_batch, data_samples = self._generate_data(batch_size=4, num_keypoints=19, norm_item=norm_item)
        ...
        nme_metric = NME(norm_mode="keypoint_distance", keypoint_indices=keypoint_indices)
    ...
    def test_exceptions_and_warnings(self):
        ...
        with self.assertRaisesRegex(KeyError, "`norm_mode` should be 'use_norm_item' or 'keypoint_distance'"):
            nme_metric = NME(norm_mode="invalid")

        # test when norm_mode = 'use_norm_item' but do not specify norm_item
        with self.assertRaisesRegex(
            KeyError, '`norm_mode` is set to `"use_norm_item"`, ' "please specify the `norm_item`"
        ):
        ...
        with self.assertRaisesRegex(
            AssertionError, "The ground truth data info do not have the expected " "normalized factor"
        ):
        ...
        with self.assertRaisesRegex(KeyError, "can not find the keypoint_indices in " "`DEFAULT_KEYPOINT_INDICES`"):
        ...
        with self.assertRaisesRegex(AssertionError, "The keypoint indices used for normalization should be a pair."):
        ...
        with self.assertRaisesRegex(AssertionError, "dataset does not contain the required"):
