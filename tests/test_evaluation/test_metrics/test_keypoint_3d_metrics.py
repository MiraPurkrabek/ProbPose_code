...
class TestMPJPE(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(self.batch_size):
            ...
            gt_instances.lifting_target_visible = np.ones((1, num_keypoints, 1)).astype(bool)

            pred_instances = InstanceData()
            pred_instances.keypoints = keypoints + np.random.normal(0, 0.01, keypoints.shape)

            data = {"inputs": None}
            data_sample = PoseDataSample(gt_instances=gt_instances, pred_instances=pred_instances)
            data_sample.set_metainfo(
                dict(target_img_path=["tests/data/h36m/S7/" "S7_Greeting.55011271/S7_Greeting.55011271_000396.jpg"])
            )
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(
            KeyError, "`mode` should be 'mpjpe', 'p-mpjpe', or 'n-mpjpe', " "but got 'invalid'."
        ):
            MPJPE(mode="invalid")
