...
class TestCocoWholeBodyMetric(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(AssertionError, "`outfile_prefix` can not be None when `format_only` is True"):
            _ = CocoWholeBodyMetric(ann_file=self.ann_file_coco, format_only=True, outfile_prefix=None)

    def test_other_methods(self):
        """test other useful methods."""
        # test `_sort_and_unique_bboxes` method
        metric_coco = CocoWholeBodyMetric(ann_file=self.ann_file_coco, score_mode="bbox", nms_mode="none")
