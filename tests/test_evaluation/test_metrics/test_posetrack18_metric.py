...
class TestPoseTrack18Metric(TestCase):
    ...
    def setUp(self):
        ...
        self.ann_file = "tests/data/posetrack18/annotations/" "test_posetrack18_val.json"
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(AssertionError, "`outfile_prefix` can not be None when `format_only` is True"):
            _ = PoseTrack18Metric(ann_file=self.ann_file, format_only=True, outfile_prefix=None)
    ...
    def test_topdown_evaluate(self):
        ...
        posetrack18_metric = PoseTrack18Metric(
            ann_file=self.ann_file, outfile_prefix=f"{self.tmp_dir.name}/test", score_mode="bbox", nms_mode="none"
        )
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "003418_mpii_test.json")))
        ...
        self.assertDictEqual(eval_results, self.target)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "009473_mpii_test.json")))
        ...
        self.assertDictEqual(eval_results, self.target)
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "012834_mpii_test.json")))

    def test_bottomup_evaluate(self):
        """test bottomup-style posetrack18 metric evaluation."""
        # case 1: score_mode='bbox', nms_mode='none'
        posetrack18_metric = PoseTrack18Metric(ann_file=self.ann_file, outfile_prefix=f"{self.tmp_dir.name}/test")
        posetrack18_metric.dataset_meta = self.posetrack18_dataset_meta
    ...
    def test_format_only(self):
        """test `format_only` option."""
        posetrack18_metric = PoseTrack18Metric(
            ann_file=self.ann_file, format_only=True, outfile_prefix=f"{self.tmp_dir.name}/test"
        )
        posetrack18_metric.dataset_meta = self.posetrack18_dataset_meta
        # process samples
        for data_batch, data_samples in self.topdown_data:
