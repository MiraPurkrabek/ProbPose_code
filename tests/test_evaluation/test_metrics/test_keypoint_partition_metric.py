...
class TestKeypointPartitionMetricWrappingCocoMetric(TestCase):
    ...
    def _convert_ann_to_bottomup_batch_data(self, ann_file):
        ...
        for img_id, anns in img2ann.items():
            _keypoints = np.array([ann["keypoints"] for ann in anns]).reshape((len(anns), -1, 3))
    ...
    def _assert_outfiles(self, prefix):
        for part in ["body", "foot", "face", "left_hand", "right_hand", "all"]:
            self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, f"{prefix}.{part}.keypoints.json")))
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, "Metrics supported by KeypointPartitionMetric"):
            _ = KeypointPartitionMetric(metric=dict(type="Metric"), partitions=dict(all=range(133)))

        # test ann_file arg warning
        with self.assertWarnsRegex(UserWarning, "does not support the ann_file argument"):
            _ = KeypointPartitionMetric(metric=dict(type="CocoMetric", ann_file=""), partitions=dict(all=range(133)))

        # test score_mode arg warning
        with self.assertWarnsRegex(UserWarning, "if score_mode is not 'bbox'"):
            _ = KeypointPartitionMetric(metric=dict(type="CocoMetric"), partitions=dict(all=range(133)))
        ...
        with self.assertRaisesRegex(AssertionError, "at least one partition"):
            _ = KeypointPartitionMetric(metric=dict(type="CocoMetric"), partitions=dict())

        with self.assertRaisesRegex(AssertionError, "should be a sequence"):
            _ = KeypointPartitionMetric(metric=dict(type="CocoMetric"), partitions=dict(all={}))
    ...
    def test_bottomup_evaluate(self):
        ...
        for key in self.target_bbox_none.keys():
            self.assertAlmostEqual(eval_results[key], self.target_bbox_none[key], places=3)
...
class TestKeypointPartitionMetricWrappingPCKAccuracy(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertWarnsRegex(UserWarning, "norm_item torso is used in JhmdbDataset"):
            _ = KeypointPartitionMetric(
                metric=dict(type="PCKAccuracy", thr=0.05, norm_item=["bbox", "torso"]), partitions=dict(all=range(133))
            )
...
class TestKeypointPartitionMetricWrappingAUCandEPE(TestCase):
    ...
    def setUp(self):
        ...
        data = {"inputs": None}
        data_sample = {"gt_instances": gt_instances.to_dict(), "pred_instances": pred_instances.to_dict()}
    ...
    def test_epe_evaluate(self):
        ...
        target = {"p1/EPE": 6.0, "p2/EPE": 17.071067810058594, "all/EPE": 11.535533905029297}
...
class TestKeypointPartitionMetricWrappingNME(TestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, "NME norm_mode 'keypoint_distance'"):
            _ = KeypointPartitionMetric(
                metric=dict(type="NME", norm_mode="keypoint_distance"), partitions=dict(all=range(133))
            )
    ...
    def test_nme_evaluate(self):
        ...
        target = {"p1/NME": 0.1715388651247378, "p2/NME": 0.4949747721354167, "all/NME": 0.333256827460395}
