...
class TestCocoMetric(TestCase):
    ...
    def setUp(self):
        ...
        self.topdown_data_coco = self._convert_ann_to_topdown_batch_data(self.ann_file_coco)
        assert len(self.topdown_data_coco) == 14
        self.bottomup_data_coco = self._convert_ann_to_bottomup_batch_data(self.ann_file_coco)
        ...
        self.dataset_meta_crowdpose["CLASSES"] = self.coco_crowdpose.loadCats(self.coco_crowdpose.getCatIds())

        self.topdown_data_crowdpose = self._convert_ann_to_topdown_batch_data(self.ann_file_crowdpose)
        ...
        self.bottomup_data_crowdpose = self._convert_ann_to_bottomup_batch_data(self.ann_file_crowdpose)
        ...
        self.dataset_meta_ap10k["CLASSES"] = self.coco_ap10k.loadCats(self.coco_ap10k.getCatIds())

        self.topdown_data_ap10k = self._convert_ann_to_topdown_batch_data(self.ann_file_ap10k)
        assert len(self.topdown_data_ap10k) == 2
        self.bottomup_data_ap10k = self._convert_ann_to_bottomup_batch_data(self.ann_file_ap10k)
    ...
    def _convert_ann_to_topdown_batch_data(self, ann_file, num_pseudo_kpts: int = 0):
        ...
        for ann in db["annotations"]:
            ...
            if num_pseudo_kpts > 0:
                keypoints = np.concatenate((keypoints, *((keypoints[:, :1],) * num_pseudo_kpts)), axis=1)
    ...
    def _convert_ann_to_bottomup_batch_data(self, ann_file):
        ...
        for img_id, anns in img2ann.items():
            keypoints = np.array([ann["keypoints"] for ann in anns]).reshape((len(anns), -1, 3))
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(AssertionError, "`outfile_prefix` can not be None when `format_only` is True"):
            _ = CocoMetric(ann_file=self.ann_file_coco, format_only=True, outfile_prefix=None)

    def test_other_methods(self):
        """test other useful methods."""
        # test `_sort_and_unique_bboxes` method
        metric_coco = CocoMetric(ann_file=self.ann_file_coco, score_mode="bbox", nms_mode="none")
    ...
    def test_format_only(self):
        ...
        with self.assertRaisesRegex(AssertionError, "Ground truth annotations are required for evaluation"):
    ...
    def test_bottomup_evaluate(self):
        ...
        metric_coco = CocoMetric(
            ann_file=self.ann_file_coco, outfile_prefix=f"{self.tmp_dir.name}/test", score_mode="bbox", nms_mode="none"
        )
        metric_coco.dataset_meta = self.dataset_meta_coco
    ...
    def test_topdown_alignment(self):
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test_align1.keypoints.json")))
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test_align2.keypoints.json")))
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test_align3.keypoints.json")))

    def test_topdown_evaluate(self):
        """test topdown-style COCO metric evaluation."""
        # case 1: score_mode='bbox', nms_mode='none'
        metric_coco = CocoMetric(
            ann_file=self.ann_file_coco, outfile_prefix=f"{self.tmp_dir.name}/test1", score_mode="bbox", nms_mode="none"
        )
        metric_coco.dataset_meta = self.dataset_meta_coco
        ...
        metric_crowdpose = CocoMetric(
            outfile_prefix=f"{self.tmp_dir.name}/test6", use_area=False, iou_type="keypoints_crowd", prefix="crowdpose"
        )
        metric_crowdpose.dataset_meta = self.dataset_meta_crowdpose
        # process samples
        for data_batch, data_samples in self.topdown_data_crowdpose:
        ...
        metric_ap10k = CocoMetric(ann_file=self.ann_file_ap10k, outfile_prefix=f"{self.tmp_dir.name}/test7")
    ...
    def test_gt_converter(self):
        ...
        self.assertTrue(osp.isfile(osp.join(self.tmp_dir.name, "test_convert.keypoints.json")))
