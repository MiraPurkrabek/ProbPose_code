...
class TestNearbyJointsNMS(TestCase):
    ...
    def test_nearby_joints_nms(self):
        ...
        kpts_db = []
        keep_pose_inds = nearby_joints_nms(kpts_db, 0.05, score_per_joint=True, max_dets=1)
        self.assertEqual(len(keep_pose_inds), 0)

        kpts_db = []
        for _ in range(5):
            kpts_db.append(dict(keypoints=np.random.rand(3, 2), score=np.random.rand(3)))
        keep_pose_inds = nearby_joints_nms(kpts_db, 0.05, score_per_joint=True, max_dets=1)
...
class TestNMSTorch(TestCase):
    ...
    def test_nms_torch(self):
        bboxes = torch.tensor([[0, 0, 3, 3], [1, 0, 3, 3], [4, 4, 6, 6]], dtype=torch.float32)
