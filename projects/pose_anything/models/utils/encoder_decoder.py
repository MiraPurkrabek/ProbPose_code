...
class ProposalGenerator(nn.Module):
    ...
    def __init__(self, hidden_dim, proj_dim, dynamic_proj_dim):
        ...
        self.dynamic_proj = nn.Sequential(
            nn.Linear(hidden_dim, dynamic_proj_dim), nn.ReLU(), nn.Linear(dynamic_proj_dim, hidden_dim)
        )
    ...
    def forward(self, query_feat, support_feat, spatial_shape):
        ...
        coord_grid = (
            torch.stack([grid_x, grid_y], dim=0).unsqueeze(0).unsqueeze(0).repeat(bs, nq, 1, 1, 1)
        )  # [bs, query, 2, h, w]
        ...
        local_max_mask = F.max_pool2d(input=max_mask, kernel_size=3, stride=1, padding=1).reshape(
            bs, nq, w * h, 1
        )  # (bs, nq, w*h, 1)
        """
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(
        ...
        decoder_layer = GraphTransformerDecoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation, normalize_before, graph_decoder
        )
        ...
        self.proposal_generator = ProposalGenerator(
            hidden_dim=d_model, proj_dim=similarity_proj_dim, dynamic_proj_dim=dynamic_proj_dim
        )
    ...
    def forward(
        ...
        query_embed, refined_support_embed = self.encoder(
            src, query_embed, src_key_padding_mask=mask, query_key_padding_mask=query_padding_mask, pos=pos_embed
        )
...
class GraphTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        tgt2 = self.self_attn(
            q, k, value=refined_support_feat, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask
        )[0]
...
class GCNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(
            in_features, out_features * kernel_size, kernel_size=1, padding=0, stride=1, dilation=1, bias=use_bias
        )
