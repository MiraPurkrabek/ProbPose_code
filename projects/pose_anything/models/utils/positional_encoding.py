...
class SinePositionalEncoding(BaseModule):
    ...
    def __init__(
        ...
        if normalize:
            assert isinstance(scale, (float, int)), (
                f"when normalize is set, " f"scale should be provided and in float or int type, " f"found {type(scale)}"
            )
    ...
    def forward(self, mask):
        ...
        y_embed = not_mask.cumsum(1, dtype=torch.float32)  # [bs, h, w], recording the y coordinate of each pixel
