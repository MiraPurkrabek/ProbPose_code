...
class Mlp(nn.Module):
    ...
    def __init__(
        self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0, mlp_fc2_bias=True
    ):
        super().__init__()
...
class WindowAttention(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, window_size={self.window_size}, "(
            f"pretrained_window_size={self.pretrained_window_size}, " f"num_heads={self.num_heads}"
        )
...
class BasicLayer(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, input_resolution={self.input_resolution}, " f"depth={self.depth}"
