...
class PoseAnythingModel(BaseModel, metaclass=ABCMeta):
    ...
    def __init__(self, keypoint_head, encoder_config, pretrained=False, train_cfg=None, test_cfg=None):
        super().__init__()
        ...
        **kwargs,
        ...
        if return_loss:
            return self.forward_train(
                img_s, target_s, target_weight_s, img_q, target_q, target_weight_q, img_metas, **kwargs
            )
        else:
    ...
    def forward_dummy(self, img_s, target_s, target_weight_s, img_q, target_q, target_weight_q, img_metas, **kwargs):
        return self.predict(img_s, target_s, target_weight_s, img_q, img_metas)

    def forward_train(self, img_s, target_s, target_weight_s, img_q, target_q, target_weight_q, img_metas, **kwargs):
        ...
        if self.with_keypoint:
            ...
            keypoint_accuracy = self.keypoint_head.get_accuracy(
                output[-1], target_keypoints, target_weight_q * mask_s, target_sizes, height=h
            )
    ...
    def forward_test(
        self, img_s, target_s, target_weight_s, img_q, target_q, target_weight_q, img_metas=None, **kwargs
    ):
        ...
        result.update({"points": torch.cat((initial_proposals, output.squeeze(1)), dim=0).cpu().numpy()})
    ...
    def extract_features(self, img_s, img_q):
        if self.backbone_type == "swin":
            ...
            ([0][:, 1:].reshape(batch_size, img_height // 8, img_width // 8, -1).permute(0, 3, 1, 2))  # [bs, 3, h, w]
            feature_s = [
    ...
    def parse_keypoints_from_img_meta(self, img_meta, device, keyword="query"):
        ...
        if keyword == "query":
            query_kpt = torch.stack(
                [torch.tensor(info[f"{keyword}_joints_3d"]).to(device) for info in img_meta], dim=0
            )[
                :, :, :2
            ]  # [bs, num_query, 2]
        else:
    ...
    def show_result(
                ...
                if pose_kpt_color is not None:
                    ...
                    for kid, kpt in enumerate(kpts):
                        ...
                        if kpt_score > kpt_score_thr:
                            ...
                            cv2.circle(img_copy, (int(x_coord), int(y_coord)), radius, (int(r), int(g), int(b)), -1)
                ...
                if skeleton is not None and pose_limb_color is not None:
                    ...
                    for sk_id, sk in enumerate(skeleton):
                        ...
                        if (
                            ...
                            transparency = max(0, min(1, 0.5 * (kpts[sk[0] - 1, 2] + kpts[sk[1] - 1, 2])))
                            cv2.addWeighted(img_copy, transparency, img, 1 - transparency, 0, dst=img)
        ...
        if show:
            ...
            enlarge = cv2.resize(img, (0, 0), fx=factor, fy=factor, interpolation=cv2.INTER_CUBIC)
