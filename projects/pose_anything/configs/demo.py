...
evaluation = dict(
    interval=25, metric=["PCK", "NME", "AUC", "EPE"], key_indicator="PCK", gpu_collect=True, res_folder=""
)
optimizer = dict(
    type="Adam",
    lr=1e-5,
...
# learning policy
lr_config = dict(policy="step", warmup="linear", warmup_iters=1000, warmup_ratio=0.001, step=[160, 180])
total_epochs = 200
...
model = dict(
    ...
    keypoint_head=dict(
        ...
        positional_encoding=dict(type="SinePositionalEncoding", num_feats=192, normalize=True),
    ),
    # training and testing settings
    train_cfg=dict(),
    test_cfg=dict(flip_test=False, post_process="default", shift_heatmap=True, modulate_kernel=11),
)
