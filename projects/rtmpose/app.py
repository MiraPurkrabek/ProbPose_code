...
models = ["rtmpose | body", "rtmo | body", "rtmpose | face", "dwpose | wholebody", "rtmw | wholebody"]
...
def predict(input, draw_heatmap=False, model_type="body", skeleton_style="mmpose", input_type="image"):
    ...
    if model_type == "rtmpose | face":
        ...
        if cached_model[model_type] is None:
            cached_model[model_type] = MMPoseInferencer(
                pose2d=os.path.join(
                    project_path, "rtmpose/wholebody_2d_keypoint/" "rtmpose-l_8xb32-270e_coco-wholebody-384x288.py"
                ),
        ...
        if cached_model[model_type] is None:
            cached_model[model_type] = MMPoseInferencer(
                pose2d=os.path.join(
                    project_path, "rtmpose/wholebody_2d_keypoint/" "rtmw-l_8xb320-270e_cocktail14-384x288.py"
                ),
                pose2d_weights="https://download.openmmlab.com/mmpose/v1/"
    ...
    if input_type == "image":
        ...
        result = next(model(input, return_vis=True, draw_heatmap=draw_heatmap, skeleton_style=skeleton_style))
        img = result["visualization"][0][..., ::-1]
        return img

    elif input_type == "video":

        for _ in model(input, vis_out_dir="test.mp4", draw_heatmap=draw_heatmap, skeleton_style=skeleton_style):
            pass
...
news_list = [
    ...
    "2023-9-25: We release an alpha version of RTMW model, the technical " "report will be released soon.",
    "2023-12-11: Update RTMW models, the online version is the RTMW-l with " "70.1 mAP on COCO-Wholebody.",
