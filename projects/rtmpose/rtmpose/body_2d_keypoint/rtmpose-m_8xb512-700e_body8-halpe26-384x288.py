...
backend_args = dict(
    backend="petrel",
    path_mapping=dict({f"{data_root}": "s3://openmmlab/datasets/", f"{data_root}": "s3://openmmlab/datasets/"}),
)
...
val_aic = dict(
    ...
    data_prefix=dict(
        img="pose/ai_challenge/ai_challenger_keypoint" "_validation_20170911/keypoint_validation_images_20170911/"
    ),
    pipeline=[dict(type="KeypointConverter", num_keypoints=num_keypoints, mapping=aic_halpe26)],
)
