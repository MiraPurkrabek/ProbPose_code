...
try:
    ...
    from .utils import blend_images, convert_video_fps, get_smoothed_kpt, resize_image_to_fixed_height
except ImportError:
    from calculate_similarity import calculate_similarity, select_piece_from_similarity
    from utils import blend_images, convert_video_fps, get_smoothed_kpt, resize_image_to_fixed_height
...
model_cfg = dict(
    human=dict(
        ...
        det_model=os.path.join(os.path.dirname(os.path.abspath(__file__)), "configs/rtmdet-nano_one-person.py"),
...
class VideoProcessor:
    ...
    def pose_estimator(self) -> Pose2DInferencer:
        ...
        if self.category not in self._pose_estimator:
            self._pose_estimator[self.category] = Pose2DInferencer(**(model_cfg[self.category]))
    ...
    def visualizer(self) -> PoseLocalVisualizer:
        ...
        metainfo_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)).rsplit(os.sep, 1)[0], "configs/_base_/datasets/coco.py"
        )
    ...
    def get_keypoints_from_frame(self, image: np.ndarray) -> np.ndarray:
        ...
        det_results = self.pose_estimator.detector(image, return_datasamples=True)["predictions"]
        ...
        data_info = dict(
            img=image, bbox=pred_instance.bboxes.cpu().numpy()[:1], bbox_score=pred_instance.scores.cpu().numpy()[:1]
        )
        ...
        keypoints = np.concatenate((pred_instances.keypoints, pred_instances.keypoint_scores[..., None]), axis=-1)
    ...
    def get_keypoints_from_video(self, video: str) -> np.ndarray:
        ...
        assert abs(video_reader.fps - 30) < 0.1, (
            f"only support videos with " f"30 FPS, but the video {video_fname} has {video_reader.fps} fps"
        )
    ...
    def run(self, tch_video: str, stu_video: str, output_file: Optional[str] = None):
        ...
        return self.generate_output_video(tch_video, stu_video, output_file, tch_kpts, stu_kpts, piece_info)
    ...
    def generate_output_video(
        ...
        for i in track_iter_progress(range(piece_info["length"])):
            ...
            tch_kpt[..., 0] += 256 - 192
            tch_kpt[..., 1] += 300 - 256
            stu_inst = InstanceData(keypoints=stu_kpt[None, :, :2], keypoint_scores=stu_kpt[None, :, 2])
            tch_inst = InstanceData(keypoints=tch_kpt[None, :, :2], keypoint_scores=tch_kpt[None, :, 2])

            stu_out_img = self.visualizer._draw_instances_kpts(np.zeros((300, 256, 3)), stu_inst)
            tch_out_img = self.visualizer._draw_instances_kpts(np.zeros((300, 256, 3)), tch_inst)
            ...
            self.visualizer.draw_texts(
                "score: ", (60, 30), font_sizes=15, colors=(255, 255, 255), vertical_alignments="bottom"
            )
            self.visualizer.draw_texts(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-file", help="Path to save the output Video", default=None)
