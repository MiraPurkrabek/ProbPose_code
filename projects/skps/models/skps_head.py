...
class SKPSHead(BaseHead):
    ...
    def __init__(
        ...
        heatmap_loss: ConfigType = dict(type="AdaptiveWingLoss", use_target_weight=True),
        offside_loss: ConfigType = dict(type="AdaptiveWingLoss", use_target_weight=True),
        ...
        if final_layer is not None:
            cfg = dict(
                type="Conv2d", in_channels=in_channels, out_channels=self.out_channels * 3, kernel_size=1, bias=True
            )
    ...
    def loss(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        # calculate losses
        losses = dict()
        heatmap_loss = self.loss_module["heatmap"](pred_heatmaps, gt_heatmaps, keypoint_weights)

        n, c, h, w = pred_offside.size()
        offside_loss_x = self.loss_module["offside"](pred_offside[:, : c // 2], gt_offside[:, : c // 2], gt_heatmaps)

        offside_loss_y = self.loss_module["offside"](pred_offside[:, c // 2 :], gt_offside[:, c // 2 :], gt_heatmaps)
        ...
        if train_cfg.get("compute_acc", True):
            _, avg_acc, _ = pose_pck_accuracy(
                output=to_numpy(pred_heatmaps), target=to_numpy(gt_heatmaps), mask=to_numpy(keypoint_weights) > 0
            )
    ...
    def predict(self, feats: Features, batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if flip_test:
            ...
            for b in range(batch_size):
                _keypoints, _keypoint_scores = self.decoder.decode(_heatmaps[b], _displacements[b])

                _keypoints_flip, _keypoint_scores_flip = self.decoder.decode(_heatmaps_flip[b], _displacements_flip[b])
    ...
    def decode(
        self, heatmaps: Tuple[Tensor], offside: Tuple[Tensor], test_cfg: ConfigType = {}, metainfo: dict = {}
    ) -> InstanceList:
        ...
        for b in range(batch_size):
            ...
            # pack outputs
            preds.append(InstanceData(keypoints=keypoints, keypoint_scores=keypoint_scores))
