...
model = dict(
    ...
    bbox_head=dict(
        in_channels=64, feat_channels=64, share_conv=False, exp_on_reg=False, use_depthwise=True, num_classes=1
    ),
    test_cfg=dict(
...
train_pipeline = [
    ...
    dict(
        type="CachedMosaic", img_scale=(input_shape, input_shape), pad_val=114.0, max_cached_images=20, random_pop=False
    ),
