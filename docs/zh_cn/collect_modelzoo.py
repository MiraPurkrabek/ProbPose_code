...
def main():
    ...
    for path in model_doc_list:
        ...
        with open(path, "r", encoding="utf-8") as f:
            doc = {"task": task, "dataset": dataset, "keywords": keywords, "path": path, "content": f.read()}
    ...
    for task, dataset_dict in model_docs.items():
        ...
        for dataset, keywords_dict in dataset_dict.items():
            ...
            for keywords, doc in keywords_dict.items():
                ...
                lines += ["<br/>", "", (f'### {" + ".join(keyword_strs)}' f" on {dataset_str}"), "", doc["content"], ""]
    ...
    for paper_cat, paper_list in paper_refs.items():
        ...
        if lines:
            ...
            with open(osp.join("model_zoo_papers", f"{paper_cat.lower()}.md"), "w", encoding="utf-8") as f:
                f.write("\n".join(lines))
