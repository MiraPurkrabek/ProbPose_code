...
def main():
    ...
    parser.add_argument(
        "--json-file", type=str, default="", help="Json file containing image person bboxes in COCO format."
    )
    parser.add_argument("--out-json-file", type=str, default="", help="Output json contains pseudolabeled annotation")
    ...
    for i in track_iter_progress(range(len(img_keys))):
        ...
        for indx, i in enumerate(pose_results):
            ...
            images = {"file_name": image_name.split("/")[-1], "height": height, "width": width, "id": int(image_id)}
