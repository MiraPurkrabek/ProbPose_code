...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='phase of dataset to visualize, accept "train" "test" and "val".' ' Defaults to "train".',
    )
...
def generate_dup_file_name(out_file):
    ...
    if out_file and osp.exists(out_file):
        ...
        exist_files = tuple(filter(lambda f: f.startswith(img_name), os.listdir(osp.dirname(out_file))))
...
def main():
    ...
    while idx < len(dataset):
        ...
        if args.mode == "original":
            if next_item is not None and item["img_path"] == next_item["img_path"]:
                # merge annotations for one image
                item["keypoints"] = np.concatenate((item["keypoints"], next_item["keypoints"]))
                item["keypoints_visible"] = np.concatenate((item["keypoints_visible"], next_item["keypoints_visible"]))
                item["bbox"] = np.concatenate((item["bbox"], next_item["bbox"]))
                progress_bar.update()
                continue
            else:
