...
def parse_args():
    ...
    parser.add_argument("--device", default="cpu", help="Device used for model initialization")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
    ...
    parser.add_argument("--input-shape", type=int, nargs="+", default=[256, 192], help="input image size")
    parser.add_argument(
        "--batch-size",
        "-b",
        type=int,
    ...
    parser.add_argument("--show-arch-info", "-s", action="store_true", help="Whether to show model arch information")
...
def inference(args, input_shape, logger):
    model = init_model(args.config, checkpoint=None, device=args.device, cfg_options=args.cfg_options)
    ...
    if args.batch_size > 1:
        outputs = {}
        avg_flops = []
        logger.info("Running get_flops with batch size specified as {}".format(args.batch_size))
        batch = batch_constructor(model, args.batch_size, input_shape)
        for i in range(args.batch_size):
            result = get_model_complexity_info(
                model, input_shape, inputs=batch["inputs"], show_table=True, show_arch=args.show_arch_info
            )
        ...
        outputs = get_model_complexity_info(
            model, input_shape, inputs=None, show_table=True, show_arch=args.show_arch_info
        )
    return outputs
...
def main():
    ...
    if args.device == "cuda:0":
        assert torch.cuda.is_available(), "No valid cuda device detected, please double check..."
