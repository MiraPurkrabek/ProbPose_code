...
def convert(video_path: str):
    ...
    print(f"ffmpeg -i {video_path} -f image2 -r 30 -b:v 5626k {image_path}/%06d.png")  # noqa
...
def split_dataset(annotation_path: str, split_path: str):
    ...
    for scene in folders:
        ...
        for aid in data.anns.keys():
            ...
            img_path = os.path.join(annotation_path.replace("annotations", "images"), scene, file_name)
        ...
        scene_train_data = dict(images=scene_train_imgs, annotations=scene_train_anns, categories=categories)
        scene_val_data = dict(images=scene_val_imgs, annotations=scene_val_anns, categories=categories)

        mmengine.dump(scene_train_data, os.path.join(annotation_path, scene, "train_annotations.json"))
        mmengine.dump(scene_val_data, os.path.join(annotation_path, scene, "val_annotations.json"))
