...
def load_trainset(data_root, out_dir):
        ...
        for seq in train_seqs:
            ...
            with open(camera_file, "r") as fin:
                ...
                for cam in train_cams:
                    ...
                    w, h = size
                    cam_param = dict(R=R, T=T, c=c, f=f, w=w, h=h, name=f"train_cam_{cam}")
            ...
            for cam in train_cams:
                ...
                joints_2d = np.reshape(annot2[cam][0][:num_frames], (num_frames, 28, 2))[:, joints_17]
                joints_3d = np.reshape(annot3[cam][0][:num_frames], (num_frames, 28, 3))[:, joints_17]
    ...
    np.savez(out_file, imgname=_imgnames, center=_centers, scale=_scales, part=_joints_2d, S=_joints_3d)
    print(f"Create annotation file for trainset: {out_file}. " f"{len(_imgnames)} samples in total.")
...
def load_testset(data_root, out_dir, valid_only=True):
    ...
    for subj in test_subjects:
        ...
        fx, cx = np.linalg.lstsq(
            annot3[:, :, [0, 2]].reshape((-1, 2)), (annot2[:, :, 0] * annot3[:, :, 2]).reshape(-1, 1), rcond=None
        )[0].flatten()
        fy, cy = np.linalg.lstsq(
            annot3[:, :, [1, 2]].reshape((-1, 2)), (annot2[:, :, 1] * annot3[:, :, 2]).reshape(-1, 1), rcond=None
        )[0].flatten()
        if subj <= 4:
            w, h = 2048, 2048
        else:
        ...
        cameras[f"TS{subj}"] = dict(
            c=np.array([[cx], [cy]]), f=np.array([[fx], [fy]]), w=w, h=h, name=f"test_cam_{subj}"
        )
        ...
        for i in valid_frames:
            ...
            shutil.copyfile(join(subj_path, "imageSequence", f"img_{i+1:06d}.jpg"), join(img_dir, imgname))
            _imgnames.append(imgname)
    ...
    np.savez(out_file, imgname=_imgnames, center=_centers, scale=_scales, part=_joints_2d, S=_joints_3d)
    print(f"Create annotation file for testset: {out_file}. " f"{len(_imgnames)} samples in total.")
