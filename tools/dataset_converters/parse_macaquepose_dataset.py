...
def save_coco_anno(data_annotation, img_root, save_path, start_img_id=0, start_ann_id=0, kpt_num=17):
    ...
    for i in range(0, len(data_annotation)):
        ...
        for kp, seg in zip(kps, segs):
            ...
            anno["bbox"] = [float(min_x), float(min_y), float(max_x - min_x + 1), float(max_y - min_y + 1)]
            anno["iscrowd"] = 0
...
save_coco_anno(
    data_annotation_train, img_root, os.path.join(dataset_dir, "annotations", "macaque_train.json"), kpt_num=17
)
save_coco_anno(
    data_annotation_val,
    img_root,
