...
class PreprocessH36m:
    ...
    def __init__(self, metadata, original_dir, extracted_dir=None, processed_dir=None, sample_rate=5):
        self.metadata = metadata
        self.original_dir = original_dir
        self.sample_rate = sample_rate

        if extracted_dir is None:
            self.extracted_dir = join(os.path.dirname(os.path.abspath(self.original_dir)), "extracted")
        else:
            self.extracted_dir = extracted_dir

        if processed_dir is None:
            self.processed_dir = join(os.path.dirname(os.path.abspath(self.original_dir)), "processed")
        else:
        ...
        self.subjects_splits = {"train": ["S1", "S5", "S6", "S7", "S8"], "test": ["S9", "S11"]}
    ...
    def generate_annotations(self):
        ...
        output_dir = join(self.processed_dir, "annotation_body3d", f"fps{50 // self.sample_rate}")
        ...
        for data_split in ("train", "test"):
            ...
            np.savez(out_file, imgname=imgnames_all, center=centers_all, scale=scales_all, part=kps2d_all, S=kps3d_all)
    ...
    def _get_camera_params(self, camera, subject):
        ...
        metadata_slice[6:] = self.cameras_raw[265 + camera * 9 - 1 : 265 + (camera + 1) * 9 - 1]
    ...
    def _load_annotations(self, subject, action, subaction, camera):
        ...
        # load 2D keypoints
        with pycdf.CDF(join(subj_dir, "MyPoseFeatures", "D2_Positions", basename + ".cdf")) as cdf:
            kps_2d = np.array(cdf["Pose"])

        num_frames = kps_2d.shape[1]
        kps_2d = kps_2d.reshape((num_frames, 32, 2))[:: self.sample_rate, self.movable_joints]
        kps_2d = np.concatenate([kps_2d, np.ones((len(kps_2d), 17, 1))], axis=2)

        # load 3D keypoints
        with pycdf.CDF(join(subj_dir, "MyPoseFeatures", "D3_Positions_mono", basename + ".cdf")) as cdf:
            kps_3d = np.array(cdf["Pose"])

        kps_3d = kps_3d.reshape((num_frames, 32, 3))[:: self.sample_rate, self.movable_joints] / 1000.0
        ...
        print(f'Annoatations for sequence "{subject} {basename}" are loaded. ' f"{len(imgnames)} samples in total.")
...
def parse_args():
    ...
    parser.add_argument("--metadata", type=str, required=True, help="Path to metadata.xml")
    parser.add_argument(
        "--original",
        type=str,
        required=True,
    ...
    parser.add_argument("--sample-rate", type=int, default=5, help="Downsample FPS to `1 / sample_rate`. Default: 5.")
