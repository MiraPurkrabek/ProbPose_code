...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert Label Studio JSON file to COCO format JSON File")
            ...
            for i, label in enumerate(item["annotations"][0]["result"]):
                ...
                if not height or not width:
                    if "original_width" not in label or "original_height" not in label:
                        logger.debug(f"original_width or original_height not found" f"in {image_name}")
                        continue
                ...
                if "rectanglelabels" == label["type"] or "labels" == label["type"]:
                    ...
                    annotations[-1]["segmentation"] = [[coord for point in points_abs for coord in point]]
                    ...
                    if i == 0 or item["annotations"][0]["result"][i - 1]["type"] != "keypointlabels":
                        annotations.append(
                            {
                                "id": annotation_id,
                                "image_id": image_id,
