        ...
        if flip_dir is None:
            ...
            if results.get("bbox_center", None) is not None:
                results["bbox_center"] = flip_bbox(
                    results["bbox_center"], image_size=(w, h), bbox_format="center", direction=flip_dir
                )
...
class RandomEdgesBlackout(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        mask, (x1, y1, x2, y2), is_textured = self._get_random_mask(
            img.shape[1], img.shape[0], results["bbox_xyxy_wrt_input"].flatten()
        )
        ...
        if not isinstance(self.encoder, list):
            ...
            if self.multilevel:
                ...
                if not all(_encoded.keys() == keys for _encoded in encoded_list):
                    raise ValueError(
                        "Encoded items from all encoders must have the same " "keys if ``multilevel==True``."
                    )
