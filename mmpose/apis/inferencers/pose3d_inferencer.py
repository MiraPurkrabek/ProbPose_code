...
class Pose3DInferencer(BaseMMPoseInferencer):
    ...
    def preprocess_single(
        ...
        results_pose2d = next(
            self.pose2d_model(
                input, bbox_thr=bbox_thr, nms_thr=nms_thr, bboxes=bboxes, merge_results=False, return_datasamples=True
            )
        )["predictions"]

        for ds in results_pose2d:
            ds.pred_instances.set_field(
                (ds.pred_instances.bboxes[..., 2:] - ds.pred_instances.bboxes[..., :2]).prod(-1), "areas"
            )
        ...
        pose_results_2d = self._pose_seq_extractor(
            self._buffer["pose_est_results_list"], frame_idx=index if self._video_input else 0
        )
        causal = self.cfg.test_dataloader.dataset.get("causal", False)
