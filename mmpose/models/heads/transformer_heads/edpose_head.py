...
class EDPoseDecoder(BaseModule):
    ...
    def __init__(
        ...
        self.kpt_index = [
            x for x in range(self.num_group * (self.num_keypoints + 1)) if x % (self.num_keypoints + 1) != 0
        ]
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                new_output_for_box = torch.gather(
                    output[effect_num_dn:], 0, topk_proposals.unsqueeze(-1).repeat(1, 1, self.embed_dims)
                )
                bs = new_output_for_box.shape[1]
                new_output_for_keypoint = (
                    new_output_for_box[:, None, :, :] + self.keypoint_embed.weight[None, :, None, :]
                )
        ...
        if num_keypoints == 17:
            if dec_pred_pose_embed_share:
                ...
                pose_embed_layerlist = [
                    copy.deepcopy(_pose_embed) for i in range(num_pred_layer - num_box_decoder_layers + 1)
                ]
        else:
            if dec_pred_pose_embed_share:
                ...
                pose_embed_layerlist = [
                    copy.deepcopy(_pose_embed) for i in range(num_pred_layer - num_box_decoder_layers)
                ]
        ...
        for dec_lid, (layer_ref_sig, layer_hs) in enumerate(zip(references[:-1], hidden_states)):
            if dec_lid < self.num_box_decoder_layers:
                ...
                vis_xy_unsig = torch.ones_like(
                    layer_outputs_unsig_keypoints, device=layer_outputs_unsig_keypoints.device
                )
...
class EDPoseHead(TransformerHead):
    ...
    def pre_transformer(self, img_feats: Tuple[Tensor], batch_data_samples: OptSampleList = None) -> Tuple[Dict]:
        ...
        if self.refine_queries_num > 0 or batch_data_samples is not None:
            input_query_label, input_query_bbox, humandet_attn_mask, human2pose_attn_mask, mask_dict = (
                self.prepare_for_denosing(batch_data_samples, device=img_feats[0].device)
            )
        else:
    ...
    def predict(self, feats: Features, batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if test_cfg.get("flip_test", False):
            assert NotImplementedError(
                "flip_test is currently not supported " "for EDPose. Please set `model.test_cfg.flip_test=False`"
            )
        else:
            pred_logits, pred_boxes, pred_keypoints = self.forward(feats, batch_data_samples)  # (B, K, D)

            pred = self.decode(
                input_shapes, pred_logits=pred_logits, pred_boxes=pred_boxes, pred_keypoints=pred_keypoints
            )
        return pred
    ...
    def prepare_for_denosing(self, targets: OptSampleList, device):
        ...
        for idx, (gt_boxes_i, gt_labels_i, gt_keypoint_i) in enumerate(zip(gt_boxes, gt_labels, gt_keypoints)):
            ...
            if num_gt_i > 0:
                ...
                gt_labels_expand_i = torch.ones(refine_queries_num, dtype=torch.int64, device=device) * int(
                    self.num_classes
                )
    ...
    def loss(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: OptConfigType = {}) -> dict:
        ...
        assert NotImplementedError(
            "the training of EDPose has not been " "supported. Please stay tuned for further update."
        )
