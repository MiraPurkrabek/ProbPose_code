...
class MotionRegressionHead(BaseHead):
    ...
    def __init__(
        ...
        self.pre_logits = nn.Sequential(
            OrderedDict([("fc", nn.Linear(in_channels, embedding_size)), ("act", nn.Tanh())])
        )
    ...
    def loss(self, inputs: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        mpjpe_err = keypoint_mpjpe(
            pred=to_numpy(pred_outputs), gt=to_numpy(lifting_target_label), mask=to_numpy(lifting_target_weight) > 0
        )
