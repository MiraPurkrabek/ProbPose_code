...
class IIAModule(BaseModule):
    ...
    def __init__(
        ...
        self.keypoint_root_conv = build_conv_layer(
            dict(type="Conv2d", in_channels=in_channels, out_channels=out_channels, kernel_size=1)
        )
...
class GFDModule(BaseModule):
    ...
    def __init__(
        ...
        self.conv_down = build_conv_layer(
            dict(type="Conv2d", in_channels=in_channels, out_channels=gfd_channels, kernel_size=1)
        )
        ...
        self.fuse_attention = build_conv_layer(
            dict(type="Conv2d", in_channels=gfd_channels * 2, out_channels=gfd_channels, kernel_size=1)
        )
        self.heatmap_conv = build_conv_layer(
            dict(type="Conv2d", in_channels=gfd_channels, out_channels=out_channels, kernel_size=1)
        )
        self.sigmoid = TruncSigmoid(min=clamp_delta, max=1 - clamp_delta)
...
class CIDHead(BaseHead):
    ...
    def predict(self, feats: Features, batch_data_samples: OptSampleList, test_cfg: ConfigType = {}) -> Predictions:
        ...
        if len(instance_coords) > 0:
            ...
            preds = [
                InstanceData(
                    keypoints=np.empty((0, self.num_keypoints, 2)), keypoint_scores=np.empty((0, self.num_keypoints))
                )
            ]
    ...
    def loss(self, feats: Tuple[Tensor], batch_data_samples: OptSampleList, train_cfg: ConfigType = {}) -> dict:
        ...
        if max_train_instances > 0 and len(instance_imgids) > max_train_instances:
            selected_indices = torch.randperm(len(instance_imgids), device=gt_heatmaps.device, dtype=torch.long)[
                :max_train_instances
            ]
        ...
        # calculate losses
        losses = {
            "loss/heatmap_coupled": self.loss_module["heatmap_coupled"](pred_heatmaps, gt_heatmaps, None, heatmap_mask)
        }
