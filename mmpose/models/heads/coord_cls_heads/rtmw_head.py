...
class RTMWHead(BaseHead):
    ...
    def __init__(
        ...
        self.mlp = nn.Sequential(
            ScaleNorm(flatten_dims), nn.Linear(flatten_dims, gau_cfg["hidden_dims"] // 2, bias=False)
        )

        self.mlp2 = nn.Sequential(
            ScaleNorm(flatten_dims * ps**2), nn.Linear(flatten_dims * ps**2, gau_cfg["hidden_dims"] // 2, bias=False)
        )
