...
class CocoMetric(BaseMetric):
    ...
    def __init__(
        ...
        if score_mode not in allowed_score_modes:
            raise ValueError(
                "`score_mode` should be one of 'bbox', 'bbox_keypoint', " f"'bbox_rle', but got {score_mode}"
            )
        ...
        if format_only:
            ...
            assert "annotations" in load(ann_file), (
                "Ground truth annotations are required for evaluation " "when `format_only` is False."
            )
            ...
            if self.coco is None:
                ...
                if self.iou_type == "keypoints_crowd":
                    assert "crowd_index" in data_sample, (
                        "`crowd_index` is required when `self.iou_type` is " "`keypoints_crowd`"
                    )
            ...
            for ann in gt_dict["raw_ann_info"]:
                ...
                if self.iou_type == "keypoints_crowd":
                    assert "num_keypoints" in ann, (
                        "`num_keypoints` is required when `self.iou_type` " "is `keypoints_crowd`"
                    )
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for img_id, instances in kpts.items():
            for instance in instances:
                ...
                instance["keypoints"] = np.concatenate(
                    [instance["keypoints"], instance["keypoint_probs"][:, None]], axis=-1
                )
        ...
        try:
            info_str = self._do_python_variables_eval(gts if self.coco is None else self.coco.anns, valid_kpts)
            name_value = OrderedDict(info_str)
            eval_results.update(name_value)

            info_str = self._do_python_vector_fields_eval(gts if self.coco is None else self.coco.anns, valid_kpts)
        ...
        try:
            self._do_oks_to_iou_eval(gts if self.coco is None else self.coco.anns, valid_kpts)
        except Exception:
        ...
        try:
            self._do_kpts_conf_eval(gts if self.coco is None else self.coco.anns, valid_kpts)
        except Exception:
    ...
    def _do_python_variables_eval(self, gts, dts):
        ...
        for key in dt_dict.keys():
            ...
            gt_abox_xywh = fix_bbox_aspect_ratio(
                gt_bbox_xywh, aspect_ratio=3 / 4, padding=self.padding, bbox_format="xywh"
            )
        ...
        print(
            "FP count: {:d} kpts, {:d} imgs ({:.2f} %)".format(
                fp_count, fp_imgs_count, fp_imgs_count / len(dt_dict) * 100
            )
        )
        print(
            "FN count: {:d} kpts, {:d} imgs ({:.2f} %)".format(
                fn_count, fn_imgs_count, fn_imgs_count / len(dt_dict) * 100
            )
        )
        print(
            "WA count: {:d} kpts, {:d} imgs ({:.2f} %)".format(
                wa_count, wa_imgs_count, wa_imgs_count / len(dt_dict) * 100
            )
        )
        print(
            "WBOX count: {:d} kpts, {:d} imgs ({:.2f} %)".format(
                wbox_count, wbox_imgs_count, wbox_imgs_count / len(dt_dict) * 100
            )
        )
        ...
        info_str = []
        vis_acc, vis_thr = self._do_classification_eval(
            gt_vis, dt_vis, force_balance=True, classification_name="Visibility", verbose=True
        )
        info_str.extend(
            [
        ...
        _, _ = self._do_classification_eval(
            gt_vis, dt_vis, force_balance=True, classification_name="Visibility", verbose=True
        )

        unique_gt_probs = np.unique(gt_probs[~np.isnan(gt_probs)])
        if len(unique_gt_probs) > 1:
            prob_acc, prob_thr = self._do_classification_eval(
                gt_probs, dt_probs, force_balance=False, verbose=True, plot_name="prob"
            )
            info_str.extend(
                [
            ...
            score_acc, score_thr = self._do_classification_eval(
                gt_probs, dt_scores, force_balance=False, verbose=True, plot_name="score"
            )
            info_str.extend(
                [
    ...
    def _do_heatmap_calibrations_eval(self, gt_kpts, dt_heatmaps, gt_aboxes):
        ...
        for b in range(B):
            ...
            scale_factor = ((abox[2:] - 1) / (np.array([W, H], dtype=np.float32) - 1)).astype(np.float32)
