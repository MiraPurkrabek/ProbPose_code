...
class SimCCVisualizer:
    ...
    def add_1d_heatmaps(
        self, maps: dict, background: np.ndarray, map2d_size: Union[tuple, list], K: int, interval: int = 10
    ):
        ...
        for i in range(K):
            ...
            cv.putText(
                background, str(i), (y_startpoint[0], y_startpoint[1] - 5), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2
            )
            x_startpoint[1] += add
            y_startpoint[0] += add
